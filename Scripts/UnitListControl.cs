...
/// <summary>
/// UnitListControl:  manages the visual depiction, and handles clicks on, units 
/// in buildings or nodes.  Somewhat coupled with the Building class. 
/// </summary>
public class UnitListControl : MonoBehaviour 
{
    /// <summary>
    ...
    /// </summary>
    private SortedList<Guid, DateTime> squadsInListControl;
    ...
    void Start () 
    {
        squadsInListControl = new SortedList<Guid, DateTime>();
    ...
    void Update () 
    {
        ...
        {
            if( !this.squadsInListControl.ContainsKey(squad.id))
            {
                // first, add it to our hash set. 
                this.squadsInListControl[squad.id] = DateTime.Now;
                ...
                textSlotPosition.y = textSlotPosition.y - (this.squadsInListControl.Count * 0.05f);
        ...
        if( this.squads.Count() < this.squadsInListControl.Count() )
        {
            ...
            // we need to identify all the nodes to remove from 'this.squadsInListControl'. Let's use a complicated, O(n^2)
            // method to do this, since I can't think of a better method right now. 
            List<Guid> squadIdsToRemove = this.squadsInListControl.Keys.Where( squadGuid => 
            {
            ...
            squadIdsToRemove.ForEach( squad => this.squadsInListControl.Remove(squad) );
    ...
    public void OnOrderAdded(Order order)
    {
        ...
        if(
            ...
            this.squadsInListControl.ContainsKey(order.SquadGuid)
